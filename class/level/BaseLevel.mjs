...
export class BaseLevel extends TypedEmitter {
	...
	addPlayer(player, position = [0, 0, 0], orientation = [0, 0]) {
		...
		const drone = new Drone(new Ego({ name: "&7" + player.getDisplayName() }))
	...
	loadPlayer(player, position = [0, 0, 0], orientation = [0, 0]) {
		player.client.loadLevel(
			...
			() => {
				...
				player.client.extensions.get("ExtendedPlayerList").configureSpawn(-1, player.getDisplayName(), position[0], position[1], position[2], orientation[0], orientation[1], player.getDisplayName())
			}
		)
	}
	/** @todo Yet to be documented. */
