		...
		for (let i = 0; i < 127; i++) {
			if (!this.server.players.some((player) => player.netId == i)) {
				...
				this.server.players.forEach((otherPlayer) => {
					player.client.extensions.get("ExtendedPlayerList").addPlayerName(otherPlayer.netId, otherPlayer.getDisplayName(), `&7${otherPlayer.getDisplayName()}`, "Server", 1)
				})
				this.server.players.push(player)
				player.client.extensions.get("ExtendedPlayerList").addPlayerName(255, player.getDisplayName(), `&7${player.getDisplayName()}`, "Server", 1)
				this.server.players.forEach((anyPlayer) => {
					if (anyPlayer != player) anyPlayer.client.extensions.get("ExtendedPlayerList").addPlayerName(i, player.getDisplayName(), `&7${player.getDisplayName()}`, "Server", 1)
				})
